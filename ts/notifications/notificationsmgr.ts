...
export class NotificationsManager {
    ...
    hasTag(tag: string) {
        if (this.showingTags && this.showingTags.includes(tag)) return true;
        for (let i = 0; i < this.notifications.length; i++) {
            if (this.notifications[i].tags.includes(tag)) return true;
        }
        return false;
    }
    push(info: Notification) {
        this.notifications.push(info);
        if (!this.showingNotification) this.run();
    }
