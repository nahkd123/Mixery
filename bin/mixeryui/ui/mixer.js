...
import { nearSnap } from "../../utils/snapper.js";
...
export class MixerTracksContainer {
    constructor(mi) {
        this.selected = { track: undefined, element: undefined };
    ...
    addMixerTrackElement(track) {
        ...
        function updateSliderThumb() {
            ...
            if (track.gain.value > 1.0 && !trackSliderThumb.classList.contains("loud"))
                trackSliderThumb.classList.add("loud");
            else if (track.gain.value <= 1.0 && trackSliderThumb.classList.contains("loud"))
                trackSliderThumb.classList.remove("loud");
        }
        ...
        if (this.mi.mixerEngine.master !== track) {
            ...
            this.masterElement = trackElement;
        trackSlider.addEventListener("mousedown", event => {
            ...
            function mouseMove(event) {
                ...
                if (!event.shiftKey)
                    newGainValue = nearSnap(newGainValue, 0.05, 0, 1, 1.5);
                if (newGainValue > 1.5)
                    newGainValue = 1.5;
                else if (newGainValue < 0)
                    newGainValue = 0;
