    ...
    applyUpdate() {
        this.canvas.addEventListener("wheel", (event) => {
            if (event.ctrlKey) {
                ...
                if (this.session.pxPerBeatTo < 25)
                    this.session.pxPerBeatTo = 25;
                if (this.session.pxPerBeatTo > this.canvas.offsetWidth - 100)
                    this.session.pxPerBeatTo = this.canvas.offsetWidth - 100;
    ...
    render() {
        ...
        for (let i = textDrawStart; i < Math.ceil(this.session.scrolledBeats + visibleBeats); i++) {
            ...
            if (this.session.pxPerBeat >= 50 || (i % 4) === 0)
                this.ctx.fillText(text, (i - textDrawStart) * this.session.pxPerBeat - charWidth / 2 - 1 - (Math.floor(this.session.scrolledBeats * this.session.pxPerBeat) % (this.session.pxPerBeat)), 23);
        }
        this.ctx.globalAlpha = 1.0;
        function drawSeekerShape(ctx, seekPx) {
            ctx.beginPath();
