    ...
    applyUpdate() {
        ...
        this.element.querySelector("div.bartrackadd").addEventListener("click", (event) => {
                ...
                function deleteClipAtCursor() {
                    ...
                    for (let i = 0; i < track.clips.length; i++) {
                        ...
                        if (beginBeat >= clip.offset && beginBeat <= clip.offset + clip.length) {
                            ...
                            if (self.session.playlist.selectedClip === clip)
                                self.session.playlist.selectedClip = undefined;
                            return;
                        }
                    }
                }
            ...
            canvas.addEventListener("mousemove", event => {
                if (buildupFriction)
                    this.session.scrollFriction -= event.movementX;
                if (!globalCanvasMouseDown)
                    return;
                ...
                if (tool === Tools.NOTHING) {
                    ...
                    if (!movingClip) {
                        ...
                        if (!resizingClip)
                            clip.offset = snap(clipOldOffset + ((event.offsetX - beginOffsetX) / this.session.pxPerBeat), ...BeatSnapPreset);
                        else
                            clip.length = snap(clipOldLength + ((event.offsetX - beginOffsetX) / this.session.pxPerBeat), ...BeatSnapPreset);
                    }
                    this.ui.canvasRenderUpdate();
                }
            });
