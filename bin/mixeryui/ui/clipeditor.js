...
export class ClipEditorInterface {
    constructor(ui) {
        this.midiClipOptions = new ContextMenu();
        this.audioClipOptions = new ContextMenu();
        this.mouse = {
            x: -1,
            y: -1,
            down: false,
    ...
    applyUpdate() {
        ...
        this.canvas.addEventListener("wheel", event => {
            if (event.ctrlKey) {
                ...
                if (this.session.clipEditor.verticalZoom < 5)
                    this.session.clipEditor.verticalZoom = 5;
                if (this.session.clipEditor.verticalZoom > 350)
                    this.session.clipEditor.verticalZoom = 350;
                event.preventDefault();
            }
            else
                this.session.clipEditor.verticalScroll += event.shiftKey ? 0 : event.deltaY;
        ...
        let oldOffset = -1;
        let automationNodeType = "linearRamp";
        ...
        this.canvas.addEventListener("mousedown", event => {
            ...
            if (this.mouse.x <= ClipEditorInterface.SIDEBAR_WIDTH)
                return;
                ...
                if (event.buttons === 4) {
                    ...
                    if (event.buttons === 1) {
                        if (node === undefined) {
                            ...
                            this.clickedAutomationNode = node;
                    }
                ...
                if (selectedTool === Tools.NOTHING) {
                    if (selectedClip instanceof MIDIClip) {
                        ...
                        if (this.clickedAutomationNode.time > 0) {
                            ...
                            if (this.clickedAutomationNode.time <= 0)
                                this.clickedAutomationNode.time = this.session.clipEditor.availableLengths[0];
                        }
                    ...
                    if (selectedClip instanceof MIDIClip) {
                        ...
                        for (let i = 0; i < selectedClip.notes.length; i++) {
                            const note = selectedClip.notes[i];
                            if (note.note === clickedNote && ((start >= note.start && start < note.start + note.duration) ||
        ...
        this.canvas.addEventListener("mouseup", event => {
            ...
            if (this.mouse.down) {
                ...
                if (selectedClip instanceof MIDIClip) {
                    if (selectedTool === Tools.NOTHING) {
                        if (clickedBeat - this.midiDrawInfo.noteStart <= 0)
                            return;
                        selectedClip.notes.push({
    ...
    drawVerticalLine(x, y1, y2) {
        if (x < ClipEditorInterface.SIDEBAR_WIDTH)
            return;
    ...
    render() {
        if (this.session.clipEditor.verticalScroll < 0)
            this.session.clipEditor.verticalScroll = 0;
        ...
        let selectedClip = this.session.playlist.selectedClip;
        if (selectedClip instanceof MIDIClip)
            this.renderMIDIClip(selectedClip);
        else if (selectedClip instanceof AudioClip)
            this.renderAudioClip(selectedClip);
        else if (selectedClip instanceof AutomationClip)
            this.renderAutomationClip(selectedClip);
    ...
    renderMIDIClip(clip) {
        ...
        if (this.session.playlist.selectedTool === Tools.NOTHING && this.mouse.down) {
            ...
            ctx.fillRect(this.midiDrawInfo.noteStart * this.session.pxPerBeat + ClipEditorInterface.SIDEBAR_WIDTH - this.session.scrolledPixels, (NotesConfiguration.NOTE_TO - this.midiDrawInfo.noteIndex) * zoom - scroll, (this.midiDrawInfo.noteEnd - this.midiDrawInfo.noteStart) * this.session.pxPerBeat, zoom);
            ctx.globalAlpha = 1;
        }
            ...
            ctx.fillRect(drawX * this.session.pxPerBeat + ClipEditorInterface.SIDEBAR_WIDTH - this.session.scrolledPixels, (NotesConfiguration.NOTE_TO - hoveringNote) * zoom - scroll, this.session.clipEditor.noteLength * this.session.pxPerBeat, zoom);
            ctx.globalAlpha = 1;
        }
    ...
    renderAudioClip(clip) {
        ...
        if (clip.renderAudioClip) {
            ...
            drawAudioBuffer(clip.cached, this.ctx, sidebarWidth + (clip.offset - this.session.scrolledBeats - clip.audioOffset) * this.session.pxPerBeat, 0, msToBeats(clip.buffer.duration * 1000, this.session.bpm) * this.session.pxPerBeat, this.canvas.height, beatsToMS(0, this.session.bpm), clip.buffer.duration * 1000, () => {
                this.ctx.stroke();
                this.ctx.fill();
            }, false);
        }
        ...
        for (let i = 0; i < clip.buffer.numberOfChannels; i++) {
            if (i !== clip.buffer.numberOfChannels - 1)
                this.drawHorizontalLine(0, sidebarWidth, (i + 1) * channelHeight);
        }
    }
    renderAutomationClip(clip) {
        const hoveringBeat = snap((this.mouse.x - ClipEditorInterface.SIDEBAR_WIDTH + this.session.scrolledPixels) / this.session.pxPerBeat, ...this.session.clipEditor.availableLengths);
        const sidebarWidth = ClipEditorInterface.SIDEBAR_WIDTH;
        const padding = 30;
        ...
        this.ctx.strokeStyle = clip.bgcolor;
        this.ctx.fillStyle = clip.bgcolor;
        drawAutomation(clip, this.ctx, sidebarWidth - (this.session.scrolledBeats - clip.offset) * this.session.pxPerBeat, padding, this.canvas.width - sidebarWidth, this.canvas.height - padding * 2, this.session.pxPerBeat, true, true);
        this.ctx.globalAlpha = 0.20;
        this.ctx.fillRect(0, 0, sidebarWidth, this.canvas.height);
        this.ctx.globalAlpha = 1;
        this.drawVerticalLine(sidebarWidth, 0, this.canvas.height);
...
ClipEditorInterface.SIDEBAR_WIDTH = 242;