    ...
    initMouseEvents() {
        ...
        this.viewCanvas.addEventListener("mousemove", event => {
            if (clickedNode !== -1) {
                ...
                switch (clickedNode) {
                    case 0:
                        this.envelope.attackTime = time;
                        break;
                    case 1:
                        this.envelope.decayTime = Math.max(time - this.envelope.attackTime, 100);
                        this.envelope.decayTo = value;
                        break;
                    case 2:
                        this.envelope.decayTo = value;
                        break;
                    case 3:
                        this.envelope.releaseTime = Math.max(time - this.envelope.attackTime - this.envelope.decayTime - 500, 0);
                        break;
                }
                this.renderEnvelope();
            }
