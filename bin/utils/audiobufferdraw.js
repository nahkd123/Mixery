function msToFrame(ms, rate = 44100) {
    return Math.floor(rate * ms / 1000);
}
function getFrameIndexByPixel(width, pixel = 0, start = 0, end = 44100) {
    const range = end - start;
    return Math.floor(start + (range * pixel / width));
}
function getFramesCountPerPixel(width, range = 44100) {
    return range / width;
}
function getPeekByPixel(buffer, width, pixel = 0, start = 0, end = 44100) {
    ...
    for (let i = 0; i < framesCountPerPX; i++)
        val = Math.max(val, Math.abs(buffer[frameIndex + i] || 0));
    return val;
}
export default function drawAudioBuffer(buffer, ctx, x, y, w, h, bufferStart, bufferEnd, pathLined = () => { }, flipChannels = true) {
