            ...
            this.clips.forEach(clip => {
                ...
                if (this.session.settings.rendering.renderNotes) {
                    ...
                    if (clip instanceof MIDIClip) {
                        ...
                        drawAudioBuffer(clip.cached, this.ctx, drawX, 14, drawW, 25, startMS, startMS + beatsToMS(clip.length, this.session.bpm), () => {
                            this.ctx.stroke();
                            this.ctx.fill();
                        });
                        this.ctx.lineWidth = 2;
...
export class Playlist {
    constructor(session) {
        this.tracks = [];
        this.selectedTool = Tools.NOTHING;
        this.selectedClip = undefined;
        this.session = session;
    }
    addTrack(name = "Track " + (this.tracks.length + 1)) {
        let track = new PlaylistTrack(this);
        track.name = name;
        this.tracks.push(track);
        return track;
    }
    findUnoccupiedTrack(location, length) {
        outer: for (let trackIndex = 0; trackIndex < this.tracks.length; trackIndex++) {
            const track = this.tracks[trackIndex];
            for (let clipIndex = 0; clipIndex < track.clips.length; clipIndex++) {
                const clip = track.clips[clipIndex];
                if ((location >= clip.offset && location <= clip.offset + clip.length) ||
